...
def BruteForce(obs, states, start_p, trans_p, emit_p):
    if len(obs) == 0:
        return []

    length = len(obs)
    possibilities = combo(states, length)
    maxProb = (0,0)
    for i in possibilities:
        prob = start_p[i[0]] * emit_p[i[0]][obs[0]]
        for x in range(1, len(i)):
            prob = prob*trans_p[i[x-1]][i[x]]*emit_p[i[x]][obs[x]]

        if prob > maxProb[-1]:
            maxProb = (i,prob)

    printable = ' '.join(maxProb[0])
    return f'The steps of states are {printable} with highest probability of {maxProb[1]}'

print(viterbi(obs, states, initial, trans_p, emit_p))
#BruteForce(obs, states, initial, trans_p, emit_p)
